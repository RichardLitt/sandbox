...
module.exports = function parse (modules, opts) {
    ...
    function finish (src) {
        ...
        (function next () {
            ...
            var test = src.slice(pos, s.start - offset)
        ...
        && node.parent.id.type === 'Identifier') {
            ...
            if (decs.length) {
                ...
                decs.forEach(function (d, i) {
                    var semicolon = (body.slice(d.end + skipOffset, d.end + skipOffset +1) === ';')
                    // console.log("semicolon", semicolon)
                    var key = (d.start + skipOffset)
                        + ',' + (d.end + skipOffset)
                    ;

                    // console.log(body.slice(d.end + skipOffset, d.end+ skipOffset + 1))
                    skip[key] = true;
                    console.log("hi")
                    var s = parse(modules, {
                        skip: skip,
                    ...
                    if (i < decs.length - 1) {
                        ...
                        console.log("comma", comma, body.slice(d.start, d.end), body.slice(decs[i+1].start, decs[i+1].end))
                        updates.push({
                            start: d.end,
